document.addEventListener('DOMContentLoaded', function() {
    console.log('🔄 DOM loaded, initializing UI with proper loading screen management');
    ...
    console.log('🔄 Loading screen properly initialized - waiting for system-ready event');
    console.log('Loading screen state:', {
        loadingScreenHidden,
        loadingOverlay: !!loadingOverlay
    });
    
    // Function to properly hide loading screen
    function hideLoadingScreen(reason) {
        if (loadingScreenHidden) {
            console.log(`🎯 Loading screen already hidden, ignoring: ${reason}`);
            return;
        }
        ...
        console.log(`🎯 Loading screen hide complete - ${reason}`);
    }
    
    // Listen for the system-ready event from the backend
    window.addEventListener('system-ready', () => {
        console.log('🚀 SYSTEM-READY EVENT RECEIVED: Backend initialization complete!');
        hideLoadingScreen('System ready event received from backend');
    });
    
    // Safety fallback - only after a reasonable time for actual initialization
    setTimeout(() => {
        if (!loadingScreenHidden) {
            console.log('⏰ SAFETY FALLBACK: Backend initialization took longer than expected');
            hideLoadingScreen('30-second safety fallback - backend may have issues');
        }
    }, 30000); // 30 seconds safety fallback
    
    // Emergency keyboard override for debugging
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape' && !loadingScreenHidden) {
            console.log('🔧 MANUAL OVERRIDE: Escape key pressed');
            hideLoadingScreen('Manual override via Escape key');
        }
    });
    ...
    if (goalForm && goalInput && userGoalDisplay && formulatedObjectiveDisplay) {
        ...
        console.error('Goal form elements not found. Check HTML IDs.');
    }
            ...
            .then(data => {
                ...
                if (currentOperationDisplay && data.current_operation_display) {
                    if (isPaused && !data.current_operation_display.toLowerCase().includes('paused')) {
                        // Keep local pause state for immediate feedback
                    } else if (!isPaused) {
